...
void send_dashboard_data(int zone, int faults, int latency, const char* msg_text) {
    if (dashboard_sock == -1) init_dashboard_udp();
    char json[512];
    const char* zname = "server";
    if (zone == 0) zname = "front";
    if (zone == 1) zname = "rear";
    if (zone == 2) zname = "left";
    if (zone == 3) zname = "right";
    
    snprintf(json, sizeof(json), "{\"latency_ns\": %d, \"sensors\": 100, \"faults\": %d, \"zone\": \"%s\", \"text\": \"%s\"}", 
             latency, faults, zname, msg_text);
...
void* central_fusion_start(void* arg) {
    ...
    send_dashboard_data(-1, 0, 0, "Fusion Engine Online. Streaming Data...");
    ...
    while (1) {
            ...
            if (pulse->code == PULSE_CODE_WATCHDOG) {
                for (int i = 0; i < NUM_ZONES; i++) {
                    ...
                    if (last_zone_seen[i] == 0 && i != ZONE_VEHICLE_VITALS) {
                        ...
                        send_dashboard_data(i, 1, 0, "[FAILSAFE] Heartbeat lost! Redundancy engaged.");
                    }
        ...
        if (msg.type == MSG_ZONAL_SUMMARY) {
            ...
            if (msg.zone_id == ZONE_VISION) {
                ...
                send_dashboard_data(msg.zone_id, 0, 48, "[TRUST MATCH] Vision vs Radar. Resolving...");
            } else if (msg.zone_id == ZONE_RADAR) {
            // SCENARIO 3: Latency vs Safety (Radar triggers fast action)
                ...
                send_dashboard_data(msg.zone_id, 0, 12, "[PRIORITY] RADAR Emergency Braking.");
            } else {
                // Routine sync for other zones
                send_dashboard_data(msg.zone_id, 0, 15, "Zonal sensors aggregated normally.");
            }
