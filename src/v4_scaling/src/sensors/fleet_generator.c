...
void* fleet_generator_start(void* arg) {
    ...
    // 1. Establish connections to all 4 Zonal Aggregators (Dynamic Resolution)
    ...
    while (g_keep_running) {
        ...
        for (int sid = 0; sid < TOTAL_SENSORS; sid++) {
            ...
            // SCENARIO 5: Sensor Failure (Simulated Hardware Fault for Zone 2)
            // Trigger failure every 20 cycles, and make it last for 6 cycles (6 seconds)
            ...
            if (cycle_count >= 4) {
                // SCENARIO 1: Time Misalignment (Sync) - (Now silenced but active)
                if (sid == 3 && (cycle_count % 7 == 0)) msg.timestamp_ns -= 150000000ULL;

                // SCENARIO 2: Conflict (Vision vs Radar) - Staggered to cycle 8, 18, 28...
